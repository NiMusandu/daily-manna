...
@router.api_route("/webhook", methods=["GET", "POST", "OPTIONS"])
async def webhook(request: Request):
    if request.method == "OPTIONS":
        return JSONResponse(content={"status": "CORS Preflight OK"}, status_code=200)

    if request.method == "GET":
        return JSONResponse(content={"message": "📡 Webhook is ready."}, status_code=200)
